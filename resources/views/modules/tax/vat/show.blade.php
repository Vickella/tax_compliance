...
<div class="max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-black/20 rounded-xl ring-1 ring-white/10 p-4">
            <div class="text-xs text-slate-400">VAT {{ ($return->vat_payable ?? 0) > 0 ? 'Payable' : 'Refundable' }}</div>
            <div class="text-xl font-bold {{ ($return->vat_payable ?? 0) > 0 ? 'text-amber-400' : 'text-emerald-400' }}">
                ${{ number_format(abs($return->vat_payable ?? 0), 2) }}
            </div>
        </div>
    </div>
