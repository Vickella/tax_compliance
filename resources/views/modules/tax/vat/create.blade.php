...
<div class="max-w-7xl mx-auto">
    ...
    <form method="POST" action="{{ route('modules.tax.vat.store') }}">
        ...
        <input type="hidden" name="vat_payable" value="{{ $calculation['vat_payable'] ?? 0 }}">
        <input type="hidden" name="vat_refundable" value="{{ $calculation['vat_refundable'] ?? 0 }}">
        <input type="hidden" name="net_vat" value="{{ $calculation['net_vat'] ?? $calculation['net_vat_payable'] ?? 0 }}">
            ...
            <div class="lg:col-span-1">
                <div class="bg-black/20 rounded-xl ring-1 ring-white/10 p-5">
                    ...
                    <div class="space-y-4">
                        ...
                        @php
                            $netPayable = $calculation['net_vat'] ?? $calculation['net_vat_payable'] ?? 0;
                        @endphp
                        <div class="p-4 rounded-lg {{ $netPayable > 0 ? 'bg-amber-600/20' : 'bg-emerald-600/20' }}">
                            <div class="text-xs text-slate-400">VAT {{ $netPayable > 0 ? 'Payable' : ($netPayable < 0 ? 'Refundable' : 'Nil') }}</div>
