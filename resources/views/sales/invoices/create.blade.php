<x-app-layout>
    <x-erp.page title="Create Sales Invoice" subtitle="Capture fiscal invoice details, VAT category, and QR payloads.">
        <x-slot name="actions">
            <x-erp.action-link href="{{ route('sales.invoices.index') }}" variant="muted">Back to Invoices</x-erp.action-link>
            <x-erp.action-button variant="primary" type="submit" form="sales-invoice-form">Save Draft</x-erp.action-button>
        </x-slot>

        <form id="sales-invoice-form" method="POST" action="{{ route('sales.invoices.store') }}" class="space-y-6">
            @csrf
            <x-erp.section>
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div>
                        <label class="text-xs text-white/70">Invoice number</label>
                        <input type="text" name="invoice_no" placeholder="SI-00002" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-2.5" required />
                    </div>
                    <div>
                        <label class="text-xs text-white/70">Invoice type</label>
                        <select name="invoice_type" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-2.5">
                    ...
                    <div>
                        ...
                        <select name="customer_id" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-2.5" required>
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-3 py-2.5" />
                    </div>
                    <div>
            ...
            <x-erp.section>
                ...
                <div class="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="fiscal_device_serial" placeholder="FD-001" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-2.5" />
                    </div>
                    <div>
                        <label class="text-xs text-white/70">Fiscal invoice number</label>
                        <input type="text" name="fiscal_invoice_number" placeholder="FTI-2026-0001" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-2.5" />
                    </div>
                    <div>
                    ...
                    <div class="lg:col-span-3">
                        ...
                        <textarea rows="3" name="qr_payload" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-2.5" placeholder="Paste or scan QR payload"></textarea>
                    </div>
                </div>
            </x-erp.section>
                                ...
                                <td class="py-3">
                                    <select name="lines[0][item_id]" class="w-full rounded-2xl bg-white/10 border border-white/10 text-white px-3 py-2.5 js-item-select" required>
                                        ...
                                        @foreach($items ?? [] as $item)
                                            <option value="{{ $item->id }}" data-selling="{{ $item->selling_price }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td class="py-3">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            itemSelect?.addEventListener('change', () => {
                ...
                if (option && option.dataset.selling) {
                    rateInput.value = option.dataset.selling;
                    recalc();
                }
            });
